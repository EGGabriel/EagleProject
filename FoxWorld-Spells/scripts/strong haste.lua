...
local combat2 = createCombatObject() -- strong haste comum
setCombatParam(combat2, COMBAT_PARAM_EFFECT, CONST_ME_MAGIC_GREEN)
setCombatParam(combat2, COMBAT_PARAM_AGGRESSIVE, 0)

local condition2 = createConditionObject(CONDITION_HASTE) -- condição do strong haste comum
...
setConditionParam(condition3, CONDITION_PARAM_TICKS, 22000)
setConditionFormula(condition3, 0.84, -66, 0.84, -66)
...
function onCastSpell(cid, var)
	...
	-- if (getPlayerStorageValue(cid,storageSkill) == TRUE and isPremium(cid) == TRUE) or getPlayerVocation(cid) == GOD then -- testa se tem a skill de aprimoramento e é Premium, ou se é GOD
		...
		if getPlayerLookDir(cid) == 1 then
			doSendDistanceShoot({x = position.x+2, y = position.y, z = position.z}, {x = position.x-3, y = position.y+1, z = position.z}, CONST_ANI_EXPLOSION)
			doSendDistanceShoot({x = position.x+2, y = position.y, z = position.z}, {x = position.x-3, y = position.y-1, z = position.z}, CONST_ANI_EXPLOSION)
			doSendDistanceShoot({x = position.x+2, y = position.y, z = position.z}, {x = position.x-3, y = position.y, z = position.z}, CONST_ANI_EXPLOSION)
		elseif getPlayerLookDir(cid) == 2 then
			doSendDistanceShoot({x = position.x, y = position.y+2, z = position.z}, {x = position.x+1, y = position.y-3, z = position.z}, CONST_ANI_EXPLOSION)
			doSendDistanceShoot({x = position.x, y = position.y+2, z = position.z}, {x = position.x-1, y = position.y-3, z = position.z}, CONST_ANI_EXPLOSION)
			doSendDistanceShoot({x = position.x, y = position.y+2, z = position.z}, {x = position.x, y = position.y-3, z = position.z}, CONST_ANI_EXPLOSION)
